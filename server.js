...
router.get('/failedPerc',function(req,res){
    pool.getConnection(function(err,connection){
    connection.query(`Select (SUM( CASE WHEN STATUS='false' Then 1 ELSE 0 END)/COUNT(*))*100 as 'FailPerc',scenarioName from RegressionRun.ScenarioRun where status='false' group by scenarioName order BY FailPerc`, function(err,rows) {
    console.log(pool._freeConnections.indexOf(connection)); // -1
    connection.release();
    console.log(pool._freeConnections.indexOf(connection)); // 0 
                if(!err) {  
